...
export function CardCommuteActions({
  ...
  const passengerStop = stops.find((stop) =>
    stop.passengers?.some(
      (p) => p.passenger?.id === currentUserId && p.status === 'ACCEPTED'
    )
  );

  const isAcceptedPassenger = !!passengerStop;
  const isConcernedByCommute = isDriver || isAcceptedPassenger;
  ...
  const showTopSection = isConcernedByCommute;

  return (
    <div
      className={cn(
        '-mx-4 flex flex-col gap-2 px-4 pb-0',
        showTopSection && 'border-t pt-2'
      )}
    >
      {showTopSection && (
