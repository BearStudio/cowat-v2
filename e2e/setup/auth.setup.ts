...
setup('authenticate as admin', async ({ page }) => {
  await Promise.all([page.to('/login'), page.login({ email: ADMIN_EMAIL })]);
  await Promise.all([
    page.waitForURL('/app'),
    expect(page.getByTestId('layout-app')).toBeVisible(),
  ]);
  await page.context().storageState({ path: ADMIN_FILE });
});

setup('authenticate as owner', async ({ page }) => {
  await Promise.all([page.to('/login'), page.login({ email: OWNER_EMAIL })]);
  await Promise.all([
    page.waitForURL('/app'),
    expect(page.getByTestId('layout-app')).toBeVisible(),
  ]);
...
setup('authenticate as user', async ({ page }) => {
  await Promise.all([page.to('/login'), page.login({ email: USER_EMAIL })]);
  await Promise.all([
    page.waitForURL('/app'),
    expect(page.getByTestId('layout-app')).toBeVisible(),
  ]);
...
setup('authenticate as invited user', async ({ page }) => {
  await Promise.all([page.to('/login'), page.login({ email: INVITED_EMAIL })]);
  ...
  try {
    await Promise.all([
      ...
      page.getByRole('textbox').first().fill('Invited User'),
    ]);
